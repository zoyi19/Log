...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_Lidar and self.use_Radar:
            cached_path = '/mnt/16THDD/hx/OpenCOOD/opencood/cached_data/LR/'
        elif self.use_Lidar:
            cached_path = '/mnt/16THDD/hx/OpenCOOD/opencood/cached_data/L/'
        elif self.use_Radar:
            cached_path = '/mnt/16THDD/hx/OpenCOOD/opencood/cached_data/R/'
        ...
        object_stack = np.concatenate(object_stack, axis=0)
    ...
    def collate_batch_train(self, batch):
        ...
        # np.array([3,5,6]), ...]}
        merged_lidar_feature_dict = self.merge_features_to_dict(processed_lidar_list)
        processed_lidar_torch_dict = \
            self.pre_processor.collate_batch(merged_lidar_feature_dict)
        ...
        # print(processed_radar_torch_dict['voxel_features'].shape)
        # print(processed_radar_torch_dict['voxel_features'].shape)
        # [2, 3, 4, ..., M], M <= max_cav
        label_torch_dict = \
