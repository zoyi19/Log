...
from opencood.models.l4dr_modules.att_bev_backbone import AttBEVBackbone_MGF


class PointPillarL4DR(nn.Module):
    def __init__(self, args):
        super(PointPillarL4DR, self).__init__()
        ...
        self.radar_pillar_vfe = PillarVFE(args['pillar_vfe'],
                                    ...
                                    voxel_size=args['voxel_size'],
                                    point_cloud_range=args['lidar_range'])
        self.backbone = AttBEVBackbone_MGF(args['base_bev_backbone'], 128)

        self.cls_head = nn.Conv2d(128 * 3 * 2 , args['anchor_number'],
                                  kernel_size=1)
        self.reg_head = nn.Conv2d(128 * 3 * 2, 7 * args['anchor_num'],
                                  kernel_size=1)
