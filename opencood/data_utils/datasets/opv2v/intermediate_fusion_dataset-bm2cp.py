...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, cav_id=None):
        ...
        """
        canvas = canvas_3d.Canvas_3D(canvas_shape=(1000, 800), left_hand=False)
        canvas_xy, valid_mask = canvas.get_canvas_coords(lidar_np)
        canvas.draw_canvas_points(canvas_xy[valid_mask])
        print('123456789')
        plt.savefig('canvas_{}.png'.format(cav_id), transparent=False, dpi=400)
        
        canvas = canvas_bev.Canvas_BEV(left_hand=False)
        canvas_xy, valid_mask = canvas.get_canvas_coords(lidar_np)
        canvas.draw_canvas_points(canvas_xy[valid_mask])
