...
from opencood.models.where2comm_modules.where2comm_attn import ScaledDotProductAttention
from opencood.models.cobevt_modules.fuse_utils import regroup as Regroup
from opencood.models.where2comm_modules.where2comm import Communication
from opencood.models.where2comm_modules.where2comm_attn import TransformerFusion
...
class AttFusion(nn.Module):
    ...
    def forward(self, xx, record_len, pairwise_t_matrix):
        print(f"Input xx shape: {xx.shape}")  # 添加这一行
        _, C, H, W = xx.shape
        B, L = pairwise_t_matrix.shape[:2]
        # print(f"record_len: {record_len}")
        split_x = regroup(xx, record_len)
        batch_node_features = split_x
        # print(f"batch_node_features[0].shape: {batch_node_features[0].shape}")
        out = []
        # iterate each batch
        for b in range(B):
            N = record_len[b]
            ...
            # update each node i
            i = 0 # ego
            # print(f"batch_node_features[{b}].shape: {batch_node_features[b].shape}")
            # print(f"t_matrix[{i}].shape: {t_matrix[i].shape}")
            # Check if batch_node_features and t_matrix match in batch size
            # if batch_node_features[b].shape[0] != t_matrix[i].shape[0]:
            #     print(f"Skipping batch {b} due to shape mismatch.")
            #     continue
