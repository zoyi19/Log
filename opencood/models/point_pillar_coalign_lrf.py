...
from opencood.models.coalign_modules.fusion_in_one import Att_w_Warp
...
from opencood.models.coalign_modules.fusion_in_one import normalize_pairwise_tfm

class PointPillarCoAlignLRF(nn.Module):
    def __init__(self, args):
        super(PointPillarCoAlignLRF, self).__init__()
        ...
        self.backbone = ResNetBEVBackbone(args['res_bev_backbone'], 128)
    ...
    def forward(self, data_dict):
        ...
        #print(f"Lidar voxel features shape: {lidar_voxel_features.shape}")
        #print(f"Radar voxel features shape: {radar_voxel_features.shape}")
        # print(f"Record length: {record_len}")
        # print(f"Lidar spatial features shape: {lidar_batch_dict['spatial_features'].shape}")
        # print(f"Radar spatial features shape: {radar_batch_dict['spatial_features'].shape}")
        ...
        #     fused_feature_list.append(fuse_module(feature_list[i], batch_dict['record_len'], normalized_affine_matrix))
